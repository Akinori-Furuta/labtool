 ...
 */
#ifndef LABTOOLDEVICE_H
#define LABTOOLDEVICE_H
...
class LabToolDevice : public Device
{
    Q_OBJECT
public:
    explicit LabToolDevice(QObject *parent = 0);
    ~LabToolDevice();

    QString name() const {return "LabTool Device";}
...
private slots:
    void handleNewConnection(LabToolDeviceComm* newComm);
    void handleConnectedStatus(bool connected);

private:
    LabToolCaptureDevice* mCaptureDevice;
    LabToolGeneratorDevice* mGeneratorDevice;

    LabToolDeviceComm*  mDeviceComm;
    LabToolDeviceCommThread* mDeviceCommThread;
